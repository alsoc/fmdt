...
./compare.py --build-path ${WD}/${build_root}/bin/ \
             ...
             --exe-args "--vid-in-path ${WD}/scripts/regression/2022_05_31_tauh_34_meteors.mp4 --ccl-impl \"LSLH\" --trk-all --cca-mag --ccl-fra-path ${out_root_dir}/%05d.pgm --log-path ${out_root_dir}" \
...
./compare.py --build-path ${WD}/${build_root}/bin/ \
             --list-exe "${list_exe_detect}" \
             --exe-args "--vid-in-path ${WD}/scripts/regression/2022_05_31_tauh_34_meteors.mp4 --ccl-impl \"LSLM\" --trk-all --cca-mag --ccl-fra-path ${out_root_dir}/%05d.pgm --log-path ${out_root_dir}" \
             --out-path "${out_root_dir}" \
             --refs-path ${WD}/scripts/regression/refs
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
