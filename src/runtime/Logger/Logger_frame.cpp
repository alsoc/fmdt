...
: Module(), frames_path(frames_path), i0(i0), i1(i1), j0(j0), j1(j1), b(b), in_img(nullptr) {
    ...
    this->create_codelet(p, [ps_in_img, ps_in_frame]
        ...
        lgr_fra.in_img[lgr_fra.i0 - lgr_fra.b] = m_in_img - (lgr_fra.j0 - lgr_fra.b);
        for (int i = lgr_fra.i0 - lgr_fra.b + 1; i <= lgr_fra.i1 + lgr_fra.b; i++)
            lgr_fra.in_img[i] = lgr_fra.in_img[i - 1] + ((lgr_fra.j1 - lgr_fra.j0) + 1 + 2 * lgr_fra.b);
