...
Video2::Video2(const std::string filename, const size_t frame_start, const size_t frame_end, const size_t frame_skip,
               ...
               const enum video_codec_hwaccel_e hwaccel, const bool ffmpeg_debug, const char* ffmpeg_in_extra_opts)
: spu::module::Stateful(), i0(0), i1(0), j0(0), j1(0), b(b), video(nullptr), img_buf(nullptr), done(false) {
    ...
    this->video = video_reader_alloc_init(filename.c_str(), frame_start, frame_end, frame_skip, bufferize,
                                          n_ffmpeg_threads, codec_type, hwaccel, PIXFMT_GRAY8, ffmpeg_debug,
    ...
    auto ps_out_img0_gray8 = this->template create_2d_socket_out<uint8_t>(p, "out_img0_gray8", img_n_rows, img_n_cols);
    auto ps_out_img1_gray8 = this->template create_2d_socket_out<uint8_t>(p, "out_img1_gray8", img_n_rows, img_n_cols);
    auto ps_out_frame = this->template create_socket_out<uint32_t>(p, "out_frame", 1);

    this->create_codelet(p, [ps_out_img0_gray8, ps_out_img1_gray8, ps_out_frame]
                            (spu::module::Module &m, spu::runtime::Task &t,const size_t frame_id) -> int {
        auto &vid2 = static_cast<Video2&>(m);

        uint8_t* out_1d_img0_gray8 = static_cast<uint8_t*>(t[ps_out_img0_gray8].get_dataptr());
        memcpy(out_1d_img0_gray8, vid2.img_buf, vid2.size_image);

        // calling get_2d_dataptr() has a small overhead (it performs the 1D to 2D conversion)
        uint8_t** out_img1_gray8 = t[ps_out_img1_gray8].get_2d_dataptr<uint8_t>(vid2.b, vid2.b);

        int cur_fra = video_reader_get_frame(vid2.video, out_img1_gray8, NULL);
        vid2.done = cur_fra == -1 ? true : false;

        uint8_t* out_1d_img1_gray8 = static_cast<uint8_t*>(t[ps_out_img1_gray8].get_dataptr());
        memcpy(vid2.img_buf, out_1d_img1_gray8, vid2.size_image);
