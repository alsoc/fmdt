...
: Module(), r_extrapol(r_extrapol), angle_max(angle_max), diff_dev(diff_dev), track_all(track_all),
  ...
  extrapol_order_max(extrapol_order_max), min_extrapol_ratio_S(min_extrapol_ratio_S), max_RoIs_size(max_RoIs_size),
  tracking_data(nullptr), BBs(nullptr) {
    ...
    auto ps_in_RoIs_magnitude = this->template create_socket_in<uint32_t>(p, "in_RoIs_magnitude", max_RoIs_size);
    ...
    this->create_codelet(p, [ps_in_frame, ps_in_RoIs_id, ps_in_RoIs_xmin, ps_in_RoIs_xmax, ps_in_RoIs_ymin,
                             ...
                             ps_in_RoIs_prev_id, ps_in_RoIs_magnitude, ps_in_n_RoIs, ps_in_motion_est]
        ...
        _tracking_perform(trk.tracking_data,
                          ...
                          static_cast<const uint32_t*>(t[ps_in_RoIs_magnitude].get_dataptr()),
                          n_RoIs,
                          &trk.BBs,
                          frame,
