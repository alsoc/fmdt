...
: Module(), i0(i0), i1(i1), j0(j0), j1(j1), b(b), S_min(S_min), S_max(S_max), max_in_RoIs_size(max_in_RoIs_size),
  max_out_RoIs_size(max_out_RoIs_size), tmp_in_RoIs_id(nullptr) {
    ...
    this->create_codelet(p, [ps_in_labels, ps_in_img_HI, ps_in_RoIs_id, ps_in_RoIs_xmin, ps_in_RoIs_xmax,
        ...
        _features_merge_CCL_HI_v2(in_labels, in_img_HI, out_labels, mrg.i0, mrg.i1, mrg.j0, mrg.j1, mrg.tmp_in_RoIs_id,
...
void Features_merger_CCL_HI_v2::init_data() {
...
void Features_merger_CCL_HI_v2::deep_copy(const Features_merger_CCL_HI_v2 &m)
{
    Module::deep_copy(m);
    this->init_data();
}
