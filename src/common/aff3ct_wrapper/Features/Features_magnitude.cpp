...
: Module(), i0(i0), i1(i1), j0(j0), j1(j1), b(b), max_RoIs_size(max_RoIs_size) {
    ...
    auto ps_out_RoIs_magn = this->template create_socket_out<uint8_t>(p, "out_RoIs_magn",
                                                                      max_RoIs_size * sizeof(RoI_magn_t));

    this->create_codelet(p, [ps_in_img, ps_in_labels, ps_in_RoIs_basic, ps_in_n_RoIs, ps_out_RoIs_magn]
        ...
        const uint32_t     in_n_RoIs     = *t[ps_in_n_RoIs    ].get_dataptr   <const uint32_t   >();
              RoI_magn_t*  out_RoIs_magn =  t[ps_out_RoIs_magn].get_dataptr   <      RoI_magn_t >();

        features_compute_magnitude(in_img, mgn.i0, mgn.i1, mgn.j0, mgn.j1, in_labels, in_RoIs_basic, out_RoIs_magn,
                                   in_n_RoIs);
