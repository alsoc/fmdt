...
int Validation_init(char* _inputs_file) {
    ...
    if (g_inputs_nb < 1) {
        ...
        VERBOSE(
            fprintf(stderr, "(DBG) [Validation] %4hu entrees dans le fichier d'input\n", (unsigned short)g_inputs_nb););
    }
    ...
    while (i < g_inputs_nb && !feof(file)) {
        if (fscanf(file, "%s %hu \t %f \t %f \t %hu \t %f \t %f \n", tmp_obj_type, &g_inputs[i].t0, &g_inputs[i].x0,
                   &g_inputs[i].y0, &g_inputs[i].t1, &g_inputs[i].x1, &g_inputs[i].y1)) {
            ...
            VERBOSE(fprintf(stderr,
                            ...
                            i, g_inputs[i].t0, g_inputs[i].x0, g_inputs[i].y0, g_inputs[i].t1, g_inputs[i].x1,
                            g_inputs[i].y1, g_inputs[i].a, g_inputs[i].b););
            ...
            if (g_inputs[i].dirX) {
                if (g_inputs[i].dirY) {
                    g_inputs[i].bb_y0 = g_inputs[i].y0 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x0 = g_inputs[i].x0 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_y1 = g_inputs[i].y1 + TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x1 = g_inputs[i].x1 + TOLERANCE_DISTANCEMIN;
                } else {
                    g_inputs[i].bb_y0 = g_inputs[i].y1 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x0 = g_inputs[i].x0 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_y1 = g_inputs[i].y0 + TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x1 = g_inputs[i].x1 + TOLERANCE_DISTANCEMIN;
                }
            } else {
                if (g_inputs[i].dirY) {
                    g_inputs[i].bb_y0 = g_inputs[i].y0 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x0 = g_inputs[i].x1 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_y1 = g_inputs[i].y1 + TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x1 = g_inputs[i].x0 + TOLERANCE_DISTANCEMIN;
                } else {
                    g_inputs[i].bb_y0 = g_inputs[i].y1 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_x0 = g_inputs[i].x1 - TOLERANCE_DISTANCEMIN;
                    g_inputs[i].bb_y1 = g_inputs[i].y0 + TOLERANCE_DISTANCEMIN;
...
void Validation_print(const Track* tracks, const int tracks_nb) {
    ...
    if (g_inputs) {
        ...
        for (int i = 0; i < g_inputs_nb; i++) {
            ...
            // tmp
            if (g_inputs[i].hits == 1)
                g_inputs[i].hits = 0; // TODO: what is this?!
            VERBOSE(fprintf(stderr,
                            "(DBG) [Validation] Input %-2d : hits = %d/%d \t nb_tracks = %3d \t %4d \t %4d \t %4d \t "
                            "%4d \t %6.1f \t %6.1f \t %6.1f \t %6.1f\n",
                            i, g_inputs[i].hits, expected_hits, g_inputs[i].nb_tracks, g_inputs[i].t0, g_inputs[i].t1,
                            g_inputs[i].track_t0, g_inputs[i].track_t1, g_inputs[i].track_x0, g_inputs[i].track_y0,
                            g_inputs[i].track_x1, g_inputs[i].track_y1););
            printf("   %3d | %s ||    %3d | %3d || %5d | %5d ||  %5d  \n", i,
                   g_obj_type_to_string_with_spaces[g_inputs[i].obj_type], g_inputs[i].hits, expected_hits,
                   g_inputs[i].t0, g_inputs[i].t1, g_inputs[i].nb_tracks);

            unsigned tmp = (g_inputs[i].hits <= expected_hits) ? g_inputs[i].hits
                                                               : expected_hits - (g_inputs[i].hits - expected_hits);
    ...
    printf("Statistics: \n");
    printf("  - Number of GT objs = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", n_gt_meteors, n_gt_stars,
           n_gt_noise, n_gt_objs);
    printf("  - Number of tracks  = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", n_track_meteors,
           n_track_stars, n_track_noise, n_tracks);
    printf("  - True positives    = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", g_positiveTrue[METEOR],
           g_positiveTrue[STAR], g_positiveTrue[NOISE], allPositiveTrue);
    printf("  - False positives   = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", g_positiveFalse[METEOR],
           g_positiveFalse[STAR], g_positiveFalse[NOISE], allPositiveFalse);
    printf("  - True negative     = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", g_negativeTrue[METEOR],
           g_negativeTrue[STAR], g_negativeTrue[NOISE], allNegativeTrue);
    printf("  - False negative    = ['meteor': %4d, 'star': %4d, 'noise': %4d, 'all': %4d]\n", g_negativeFalse[METEOR],
           g_negativeFalse[STAR], g_negativeFalse[NOISE], allNegativeFalse);
