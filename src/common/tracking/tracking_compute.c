...
size_t _search_valid_motion_id(const History_t* history, const size_t start_index) {
    assert(start_index < history->_size);
    for (size_t i = start_index; i < history->_size; i++) {
        if (!isnan(history->motion[i].tx) && !isnan(history->motion[i].ty))
            return i;
    }
    fprintf(stderr, "(EE) No valid motion found in '_search_valid_motion_id' function, exiting...");
    exit(-1);
}
...
void _compute_angle_and_norms(const History_t* history, const track_t* cur_track, float *angle_degree,
    ...
    size_t motion_id0 = _search_valid_motion_id(history, 0);
    float theta0 = history->motion[motion_id0].theta;
    float tx0 = history->motion[motion_id0].tx;
    float ty0 = history->motion[motion_id0].ty;

    size_t motion_id1 = _search_valid_motion_id(history, 1);
    float theta1 = history->motion[motion_id1].theta;
    float tx1 = history->motion[motion_id1].tx;
    float ty1 = history->motion[motion_id1].ty;
...
void _tracking_perform(tracking_data_t* tracking_data, const uint32_t* RoIs_id, const uint32_t* RoIs_xmin,
    ...
    _light_copy_RoIs(RoIs_id, frame, RoIs_xmin, RoIs_xmax, RoIs_ymin, RoIs_ymax, RoIs_S, RoIs_x, RoIs_y, RoIs_error,
                     RoIs_prev_id, n_RoIs, tracking_data->history->RoIs[0]);
