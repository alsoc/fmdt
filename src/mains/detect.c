...
int main(int argc, char** argv) {
    ...
    if (args_find(argc, argv, "--help,-h")) {
        ...
        fprintf(stderr,
                ...
                def_p_knn_s);
        fprintf(stderr,
                "  --mtn-no-reg        Disable motion registration (for ground detection)                        \n");
        fprintf(stderr,
    ...
    const int p_mtn_no_reg = args_find(argc, argv, "--mtn-no-reg, --mtn-no-registration");
    ...
    while ((cur_fra = video_reader_get_frame(video, I, NULL)) != -1) {
        ...
        // step 2: CCL/CCA
        RoIs_tmp->_size = CCL_apply(ccl_data, (const uint8_t**)IL, L1, 0);
        ...
        kNN_match(knn_data, RoIs0->basic, RoIs0->asso, RoIs0->_size, RoIs1->basic, RoIs1->asso, RoIs1->_size, p_knn_k,
                  p_knn_d, p_knn_s);
